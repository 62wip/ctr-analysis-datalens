...
Чарты с первых двух вкладок макета дашборда требуют только полный набор данных: всю агрегацию и предобработку данных можно выполнить на самой платформе Yandex DataLens. Третья вкладка требует метрик и сводных данных от обученной модели CatBoost.
...
В итоге в качестве СУБД был выбран ClickHouse, поскольку Яндекс сам рекомендует его для работы с Yandex DataLens. Кроме того, важным фактором стал доступный бесплатный период использования, что позволило развернуть хранилище и протестировать интеграцию без дополнительных затрат. Колоночная структура ClickHouse также обеспечивает высокую скорость выполнения аналитических запросов, что особенно важно при работе с большими объемами данных и построении интерактивных дашбордов.

== Подготовка ClickHouse и загрузка данных

После регистрации на платформе ClickHouse был создан сервис с самой дешевой конфигурацией.
...
Листинг @lst:lift_calculation содержит код для подсчета Lift метрики для категорий. Стоит отметить, что одновременно с подсчетом пользы конкретного значения категории также собиралась доля трафика, которая относится к этой категории.
...
В этом блоке был рассчитан средний реальный CTR и средний предсказанный CTR в группировке по следующим признакам: `site_category`, `app_category`, `device_model`, `device_type`. Результат сохранен в файл `diff_ctr.csv`.
...
diff_ctr_df = pd.concat(diff_ctr_data, ignore_index=True)
  ```,
  caption: [Вычисления среднего реального и предсказанного CTR в группировке по разным признакам],
...
Из подключений были созданы все необходимые для чартов датасеты. Они представлены на Рисунке @fig:datelens_datasets.
