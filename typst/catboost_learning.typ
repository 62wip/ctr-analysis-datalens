...
#figure(
  ...
  ```,
  caption: [Функция `data_tranformer` для предобработки данных],
) <lst:data_tranformer>
...
- `learning_rate=0.03` -- скорость обучения (шаг градиентного спуска). Подбиралось империческим путем по количеству итераций.
...
#figure(
  ...
  caption: [Результаты посылки решения на Kaggle],
